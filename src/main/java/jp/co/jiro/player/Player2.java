...
public class Player2 implements Player {
    ...
    public static final int BIT_FLAG = 2;

    public Player2(int rowMax, int colMax, ToggleGroup group) {
        this.group = group;

        fuList = new LinkedList<ShougiCellBuilder>() {{
            for (int i = 0; i < 9; i++) {
                add((row, col) -> new Fu(row, col, new Player2(rowMax, colMax, group)));
            }
        }};

        kinList = new LinkedList<ShougiCellBuilder>() {{
            for (int i = 0; i < 2; i++) {
                add((row, col) -> new Kin(row, col, new Player2(rowMax, colMax, group)));
            }
        }};

        ginList = new LinkedList<ShougiCellBuilder>() {{
            for (int i = 0; i < 2; i++) {
                add((row, col) -> new Gin(row, col, new Player2(rowMax, colMax, group)));
            }
        }};

        keimaList = new LinkedList<ShougiCellBuilder>() {{
            for (int i = 0; i < 2; i++) {
                add((row, col) -> new Keima(row, col, new Player2(rowMax, colMax, group)));
            }
        }};

        kakuList = new LinkedList<ShougiCellBuilder>() {{
            for (int i = 0; i < 1; i++) {
                add((row, col) -> new Kaku(row, col, new Player2(rowMax, colMax, group), rowMax, colMax));
            }
        }};

        hishaList = new LinkedList<ShougiCellBuilder>() {{
            for (int i = 0; i < 1; i++) {
                add((row, col) -> new Hisha(row, col, new Player2(rowMax, colMax, group), rowMax, colMax));
            }
        }};

        kyoshaList = new LinkedList<ShougiCellBuilder>() {{
            for (int i = 0; i < 2; i++) {
                add((row, col) -> new Kyosha(row, col, new Player2(rowMax, colMax, group), rowMax));
            }
        }};

        ouList = new LinkedList<ShougiCellBuilder>() {{
            for (int i = 0; i < 1; i++) {
                add((row, col) -> new Ou(row, col, new Player2(rowMax, colMax, group)));
            }
        }};
    ...
    public List<ShougiCell> calcRyodoOfKyosha(ShougiCell cell, int rowMax) {
        return PlayerUtil.calcRyodoOfKyosha(cell, this, cell.getRow(), rowMax);
    }
